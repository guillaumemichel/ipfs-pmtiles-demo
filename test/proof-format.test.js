...
import {
  ...
  encodeMeta,
  encodeShard,
} from '../scripts/lib/proof-encode.js';
import {
  decodeFontProofs,
  decodeMeta,
  KIND_DIR,
...
import { toHex, VerificationError } from '../src/verify.js';
import { deterministicBytes } from './helpers.js';

const digest = (seed) => deterministicBytes(32, seed);
...
test('shard validation rejects malformed structure', () => {
  ...
  // Encoder enforces the same rules.
...
test('meta range decode returns only covering entries, coverage stays whole-file', () => {
  ...
  assert.deepEqual(tail.entries.map((e) => e.start), [500]);
...
test('font proofs round-trip sorted UTF-8 paths', () => {
  const records = [
    { path: 'Noto Sans Regular/0-255.pbf', digest: digest(8) },
    { path: 'Noto Sans Regular/256-511.pbf', digest: digest(9) },
  ];
  const map = decodeFontProofs(encodeFontProofs([records[1], records[0]]));
  assert.deepEqual(
    [...map.entries()],
    records.map((r) => [r.path, toHex(r.digest)]),
  );
});

test('font proofs reject duplicates, disorder, and bad UTF-8', () => {
  const one = { path: 'a', digest: digest(10) };
  assert.throws(() => encodeFontProofs([one, one]), /duplicate/);

  // Record layout: u16le pathLength, path, digest32 — 'a' is at byte 2 of
  // the first 35-byte record, 'b' at byte 2 of the second.
  const ordered = encodeFontProofs([one, { path: 'b', digest: digest(11) }]);
  const swapped = new Uint8Array(ordered);
  [swapped[2], swapped[37]] = [swapped[37], swapped[2]]; // 'a' <-> 'b'
  assert.throws(() => decodeFontProofs(swapped), /ascending/);

  const utf8 = new Uint8Array(encodeFontProofs([one]));
  utf8[2] = 0xff; // lone continuation byte in the path
  assert.throws(() => decodeFontProofs(utf8), /UTF-8/);

  const overlong = new Uint8Array(encodeFontProofs([one]));
  overlong[1] = 0xff; // pathLength high byte: claims a 65281-byte path
  assert.throws(() => decodeFontProofs(overlong), /truncated/);

  assert.throws(() => decodeFontProofs(new Uint8Array(0)), /empty/);
});
