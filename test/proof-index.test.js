...
import { encodeMeta, encodeShard } from '../scripts/lib/proof-encode.js';
import { KIND_SHARD } from '../src/proof-format.js';
...
function indexOver(fixture, opts = {}) {
...
test('full-range descent recovers every leaf digest', async () => {
  const fixture = await testMapBundle({ mapBytes, cuts });
  const { index } = indexOver(fixture);
  assert.deepEqual(await index.leavesFor(0, 4000), asHex(fixture.dag.leaves));
});

test('sub-range descent returns exactly the covering leaves', async () => {
  const fixture = await testMapBundle({ mapBytes, cuts });
  const { index } = indexOver(fixture);
  const all = asHex(fixture.dag.leaves);
...
test('a nested tree descends lazily: only covering shards are fetched', async () => {
  ...
  const fixture = await testMapBundle({
  ...
  assert.deepEqual(tail, asHex(fixture.dag.leaves).slice(-1));
...
test('a shard inconsistent with its meta entry is rejected', async () => {
  ...
  const meta = encodeMeta([
    { kind: KIND_SHARD, length: 50, digest: new Uint8Array(createHash('sha256').update(shard).digest()) },
  ]);
  const metaDigest = createHash('sha256').update(meta).digest('hex');
  const store = new VerifiedStore([
    new RangeSource('.', {
      fetchFn: rangeFetch(
        new Map([
  ...
  const index = new ProofIndex(store, { dir: 'proofs', metaDigest, fileSize: 50 });
...
test('a meta whose coverage disagrees with its parent is rejected', async () => {
  ...
  const index = new ProofIndex(store, { dir: 'proofs', metaDigest, fileSize: 100 });
