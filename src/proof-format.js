...
// spec; the encoders live with the build in scripts/lib/proof-encode.js.
...
// sha2-256 outputs (the algorithm is declared once in metadata.json, "hash");
...
//   font proofs   := ( u16le(pathLength ≥ 1) path:utf8 digest32 )+
//
...
import { toHex, VerificationError } from './verify.js';
...
export function validateShard(bytes, spanLength, label) {
  if (bytes.length === 0) throw new VerificationError(`${label}: empty shard`);
  if (bytes.length % SHARD_RECORD_SIZE !== 0) {
    throw new VerificationError(`${label}: size ${bytes.length} not a multiple of ${SHARD_RECORD_SIZE}`);
  }
...
export function shardLeavesFor(bytes, startOffset, spanLength, start = 0, end = Infinity) {
  const count = bytes.length / SHARD_RECORD_SIZE;
...
function relOffsetAt(bytes, index) {
  const pos = index * SHARD_RECORD_SIZE;
  return bytes[pos] + bytes[pos + 1] * 0x100 + bytes[pos + 2] * 0x10000 + bytes[pos + 3] * 0x1000000;
}

// -> { entries: [{kind, start, length, digest(hex)}], covered }. Same range
// semantics as shardLeavesFor: only entries overlapping [start, end) are
// materialized; `covered` sums every entry. Entry starts are prefix sums
// from startOffset (the directory's range start; 0 for the top directory).
export function decodeMeta(bytes, startOffset, start = 0, end = Infinity) {
  ...
  for (let pos = 0; pos < bytes.length; pos += META_RECORD_SIZE) {
    ...
    if (offset + length > start && offset < end) {
      const at = pos + 9;
      entries.push({ kind, start: offset, length, digest: toHex(bytes.subarray(at, at + DIGEST_LENGTH)) });
    }
    offset += length;
  }
...
function readU64(bytes, pos, label) {
  const lo = bytes[pos] + bytes[pos + 1] * 0x100 + bytes[pos + 2] * 0x10000 + bytes[pos + 3] * 0x1000000;
  const hi = bytes[pos + 4] + bytes[pos + 5] * 0x100 + bytes[pos + 6] * 0x10000 + bytes[pos + 7] * 0x1000000;
  if (hi >= 0x200000) throw new VerificationError(`${label}: value exceeds 2^53`);
  return lo + hi * 0x100000000;
}

// -> Map(path -> digest(hex)); paths are decoded UTF-8, relative to the fonts
// directory, and must be strictly ascending (rejects duplicates, pins order).
export function decodeFontProofs(bytes) {
  const decoder = new TextDecoder('utf-8', { fatal: true });
  const map = new Map();
  let pos = 0;
  let prev = '';
  while (pos < bytes.length) {
    if (pos + 2 > bytes.length) throw new VerificationError('font proofs: truncated record');
    const pathLength = bytes[pos] + bytes[pos + 1] * 0x100;
    pos += 2;
    if (pathLength === 0) throw new VerificationError('font proofs: empty path');
    if (pos + pathLength + DIGEST_LENGTH > bytes.length) {
      throw new VerificationError('font proofs: truncated record');
    }
    let path;
    try {
      path = decoder.decode(bytes.subarray(pos, pos + pathLength));
    } catch {
      throw new VerificationError('font proofs: invalid UTF-8 path');
    }
    pos += pathLength;
    if (path <= prev) {
      throw new VerificationError('font proofs: paths not strictly ascending');
    }
    prev = path;
    map.set(path, toHex(bytes.subarray(pos, pos + DIGEST_LENGTH)));
    pos += DIGEST_LENGTH;
  }
  if (map.size === 0) throw new VerificationError('font proofs: empty file');
  return map;
}
