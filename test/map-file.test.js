...
import { MapFile } from '../src/map-file.js';
...
// The full verified read stack over an in-memory dumb host, built by the
// real pipeline: proofs descent → runs → Range requests → digest checks.
async function openFixture(data, cuts, { tamper, onRequest, ...bundleOpts } = {}) {
  const fixture = await testMapBundle({ mapBytes: data, cuts, ...bundleOpts });
...
const readsAgree = async (file, data) => {
...
test('reads agree through a nested proofs tree', async () => {
  ...
  const { file } = await openFixture(data, Array.from({ length: 40 }, () => 100), {
    shardCap: 40,
    metaMaxEntries: 4,
  });
  await readsAgree(file, data);
...
test('a cold leaf-aligned read races the tile fetch against the proof descent', async () => {
  ...
  const log = [];
  const { file } = await openFixture(data, [100, 900, 2000], {
    onRequest: (url, h) => log.push(h?.Range ? `range:${h.Range}` : url.replace(/^.*proofs/, 'proofs')),
  });
  ...
  assert.ok(log.some((l) => l.startsWith('proofs')), 'proof descent ran in parallel');
...
test('a range response longer than the run is cut off before hashing', async () => {
  ...
  await assert.rejects(file.read(0, 400), (err) => {
    assert.match(err.errors[0].message, /exceeds expected length/);
    return true;
  });
});
