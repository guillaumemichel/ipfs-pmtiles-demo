...
import { deterministicBytes, flipByte } from './helpers.js';

const sha256Hex = (bytes) => createHash('sha256').update(bytes).digest('hex');

function chunk(length, seed, offset = 0) {
...
function source({ files = new Map(), buffer = new Uint8Array(0) } = {}, { fail, tamper, onRequest } = {}) {
  return {
...
test('a run body of the wrong length is rejected before hashing', async () => {
  ...
  crypto.subtle.digest = (...args) => ((hashed = true), subtle(...args));
  try {
    await assert.rejects(store.fetchRun('map', [leaf]), (err) => {
      assert.match(err.errors[0].message, /got 50 bytes, expected 100/);
      return true;
    });
...
test('evicts least-recently-used bytes beyond the cache cap', async () => {
  ...
  const b = chunk(80, 11);
  let requests = 0;
  const files = new Map([['a', a.bytes], ['b', b.bytes]]);
