...
} from '../src/range-source.js';
import { VerificationError } from '../src/verify.js';
import { deterministicBytes, rangeFetch } from './helpers.js';
...
test('a 200 to a ranged request is refused (host ignores Range)', async () => {
  const source = new RangeSource('.', {
    fetchFn: async () => new Response(new Uint8Array(data), { status: 200 }),
  });
...
test('a 206 body longer than the requested run is cut off and rejected', async () => {
  const source = new RangeSource('.', {
    fetchFn: async () => new Response(new Uint8Array(data), { status: 206 }),
  });
...
test('a ranged TypeError after a good plain GET is the CORS-preflight diagnostic', async () => {
  const source = new RangeSource('.', {
    fetchFn: async (_url, init) => {
      if (init?.headers?.Range) throw new TypeError('NetworkError'); // preflight blocked
